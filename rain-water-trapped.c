...
int trap(const int* A, int n1) {
    ...
    for(i=0;i<n1;i++)
        count+=B[i];
    int *C=(int*)malloc(n1*sizeof(A));
    for(i=0;i<n1;i++)
        C[i]=A[i];
    //printf("check1\n");
    while(count>1){
        //printf("check2\tcount=%d\n",count);
        i=0;j=0;
        while(j<n1){
            //printf("check3\ti=%d\tj=%d\n",i,j);
            while((B[i]!=1)&&(i<n1)){
                i++;
            }
            if(i==n1)
